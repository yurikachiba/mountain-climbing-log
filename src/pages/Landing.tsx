...
const features = [
  ...
  {
    title: 'AI分析（5種類）',
    desc: 'Claude APIで日記を深層分析。「今日」は友人視点で深く読む。「今日の景色」はフィルターなしで全トピックをマッピング。「急所」は本質を突く。「外基準の統合」は内側を守ったまま外基準を扱えているかを診る。「自然の眼」は日記の中の比喩・メタファーを拾い上げ、世界の捉え方を読む。',
    icon: '🤖',
    link: '/analysis',
  },
  {
...
const faqs = [
  ...
  },
  {
    ...
    a: '「今日」「今日の景色」「急所」「外基準の統合」「自然の眼」の5種類の深層分析に対応しています。「今日」は友人視点で今日の日記だけを深く読む分析。「今日の景色」はフィルターなしで今日の全トピックをマッピングする分析。「急所」は今日の日記から本質を突く指摘。「外基準の統合」は内側を守ったまま外基準を道具として扱えているかを構造化する分析。「自然の眼」は日記の中の比喩・メタファーを拾い上げ、世界の捉え方を読み解く分析です。ユーザー自身のAnthropic APIキーを使用するため、データがサービス提供者に渡ることはありません。',
  },
  {
    q: 'スマートフォンでも使えますか？',
