...
const features = [
  ...
  {
    title: 'AI分析（4種類）',
    desc: 'OpenAI APIで日記を深層分析。「今日」は友人視点で深く読む。「今日の景色」はフィルターなしで全トピックをマッピング。「急所」は直近1週間から本質を突く。「外基準の統合」は内側を守ったまま外基準を道具として扱えているかを診る。',
    icon: '🤖',
    link: '/analysis',
  },
  {
...
const faqs = [
  ...
  },
  {
    ...
    a: '「今日」「今日の景色」「急所」「外基準の統合」の4種類の深層分析に対応しています。「今日」は友人視点で今日の日記だけを深く読む分析。「今日の景色」はフィルターなしで今日の全トピックをマッピングする分析。「急所」は直近1週間から本質を突く指摘。「外基準の統合」は今日の日記から、内側を守ったまま外基準を道具として扱えているかを構造化する分析です。ユーザー自身のOpenAI APIキーを使用するため、データがサービス提供者に渡ることはありません。',
  },
  {
    q: 'スマートフォンでも使えますか？',
      ...
      <section className="landing-section" id="details">
        ...
        <div className="landing-detail-block">
          ...
          <p>
            ...
            「急所」は直近1週間から本質を突くたった一つの指摘。「外基準の統合」は今日の日記から、内側を守ったまま外基準を道具として扱えているかを構造化する。
