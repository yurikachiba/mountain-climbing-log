...
const features = [
  ...
  {
    title: 'AI分析（6種類）',
    desc: 'Claude APIで日記を深層分析。「今日」は友人視点で深く読む。「今日の景色」はフィルターなしで全トピックの並びを見せる。「急所」は本質を突く。「外基準の統合」は内側を守ったまま外基準を扱えているかを診る。「自然の眼」は比喩・メタファーから世界の捉え方を読む。「時間の地層」は3日〜5年の6つの距離から変化を描く。',
    icon: '🤖',
    link: '/analysis',
  },
  {
...
const faqs = [
  ...
  },
  {
    ...
    a: '「今日」「今日の景色」「急所」「外基準の統合」「自然の眼」「時間の地層」の6種類の深層分析に対応しています。「今日」は友人視点で今日の日記だけを深く読む分析。「今日の景色」は今日の全トピックの並びと温度差を見せる分析。「急所」は今日の日記から本質を突く指摘。「外基準の統合」は内側を守ったまま外基準を道具として扱えているかを構造化する分析。「自然の眼」は日記の中の比喩・メタファーを拾い上げ、世界の捉え方を読み解く分析。「時間の地層」は3日・1週間・1ヶ月・1年・3年・5年の6つの距離から今日を見て、変化の事実を評価せずに描く分析です。ユーザー自身のAnthropic APIキーを使用するため、データがサービス提供者に渡ることはありません。',
  },
  {
    q: 'スマートフォンでも使えますか？',
      ...
      <section className="landing-section" id="details">
        ...
        <div className="landing-detail-block">
          ...
          <p>
            ...
            「時間の地層」は3日・1週間・1ヶ月・1年・3年・5年の6つの距離から今日を見て、変化の事実を評価せずに描く。
