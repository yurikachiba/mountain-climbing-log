...
const features = [
  ...
  {
    title: 'AI分析（9種類）',
    desc: 'Claude APIで日記を深層分析。「今日」「今日の景色」「急所」「外基準の統合」「自然の眼」で今日を多角的に読み、「時間の地層」で時間軸の変化を描く。さらに「文体気象図」「反復の地図」「季節の自分」で複数エントリを横断して自分のパターンを発見する。',
    icon: '🤖',
    link: '/analysis',
  },
  {
...
const faqs = [
  ...
  },
  {
    ...
    a: '9種類の深層分析に対応しています。「今ここ」カテゴリでは「今日」「今日の景色」「急所」「外基準の統合」「自然の眼」の5種類で今日の日記を多角的に読みます。「時間」カテゴリの「時間の地層」は3日〜5年の8つの距離から変化を描きます。「横断」カテゴリでは「文体気象図」が感情状態ごとの文体パターンを読み、「反復の地図」が繰り返し現れるテーマをマッピングし、「季節の自分」が春夏秋冬ごとの傾向を描きます。ユーザー自身のAnthropic APIキーを使用するため、データがサービス提供者に渡ることはありません。',
  },
  {
    q: 'スマートフォンでも使えますか？',
      ...
      <section className="landing-section" id="details">
        ...
        <div className="landing-detail-block">
          ...
          <p>
            ...
            「急所」は今日の日記から本質を突くたった一つの指摘。「外基準の統合」は内側を守ったまま外基準を道具として扱えているかを構造化する。
            「自然の眼」は比喩・メタファーから世界の捉え方を読み解く。
            「時間の地層」は3日〜5年の8つの距離から変化を描く。
            さらに複数エントリ横断分析として、「文体気象図」は感情状態ごとの文体パターンを読み、
            「反復の地図」は繰り返し現れるテーマ・言い回しをマッピングし、
            「季節の自分」は春夏秋冬ごとの自分の傾向を描く。
