...
export function WebAppJsonLd() {
  const data = {
    ...
    featureList: [
      ...
      'ワードクラウド（語彙傾向の可視化）',
      'AI分析9種類（今日・今日の景色・急所・外基準の統合・自然の眼・時間の地層・文体気象図・反復の地図・季節の自分 ― Claude API連携）',
      '観測所（やさしい問いかけジャーナリング）',
      '宝物庫（お気に入りの一文を保存）',
      'オフライン対応（PWA）',
      'データエクスポート・バックアップ機能',
  ...
  const data = {
    ...
    step: [
      ...
      },
      {
        ...
        text: 'タイムラインで感情の推移を追う、ワードクラウドで語彙の傾向を把握する、AI分析（9種類）で客観的な視点を得る。日記が自己分析ツールになります。',
  ...
  const data = {
    ...
    mainEntity: [
      ...
      },
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: '9種類の深層分析に対応しています。「今ここ」カテゴリ（今日・今日の景色・急所・外基準の統合・自然の眼）で今日の日記を多角的に読み、「時間」カテゴリ（時間の地層）で3日〜5年の変化を描き、「横断」カテゴリ（文体気象図・反復の地図・季節の自分）で複数エントリを横断して自分のパターンを発見します。ユーザー自身のAnthropic APIキーを使用するため、データがサービス提供者に渡ることはありません。',
        },
      },
      {
        '@type': 'Question',
