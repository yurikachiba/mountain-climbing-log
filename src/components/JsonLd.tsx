...
export function WebAppJsonLd() {
  const data = {
    ...
    featureList: [
      ...
      'ワードクラウド（語彙傾向の可視化）',
      'AI分析4種類（今日・今日の景色・急所・外基準の統合 ― 今日の日記ベース、OpenAI API連携）',
      '観測所（やさしい問いかけジャーナリング）',
      '宝物庫（お気に入りの一文を保存）',
      'オフライン対応（PWA）',
      'データエクスポート・バックアップ機能',
  ...
  const data = {
    ...
    mainEntity: [
      ...
      },
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: '「今日」「今日の景色」「急所」「外基準の統合」の4種類の深層分析に対応しています。「今日」は友人視点で今日の日記だけを深く読む分析、「今日の景色」はフィルターなしで今日の全トピックをマッピングする分析、「急所」は直近1週間から本質を突く指摘、「外基準の統合」は今日の日記から内側を守ったまま外基準を道具として扱えているかを構造化する分析です。ユーザー自身のOpenAI APIキーを使用するため、データがサービス提供者に渡ることはありません。',
        },
      },
      {
        '@type': 'Question',
