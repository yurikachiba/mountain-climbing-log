...
export function WebAppJsonLd() {
  const data = {
    ...
    featureList: [
      '日記インポート（テキスト・Markdown・JSON対応）',
      '自動日付認識（複数の日付形式に対応）',
      'ランダム日記表示（他人モード搭載）',
      '過去の同じ日の振り返り（年をまたいだ比較）',
      'キーワード全文検索（日付範囲絞り込み対応）',
      'カレンダーヒートマップ表示（年別・月別俯瞰）',
      'ワードクラウド可視化（語彙傾向の把握）',
      'タイムライン・感情分析グラフ（標高メタファー搭載）',
      'AI分析12種類（OpenAI API連携・ユーザーAPIキー使用）',
      '宝物庫（お気に入りの文章を保存）',
      'オフライン対応（PWA）',
      'データエクスポート・バックアップ機能',
    ],
  ...
  const data = {
    ...
    step: [
      ...
      },
      {
        ...
        text: 'タイムラインで感情の推移を追う、ワードクラウドで語彙の傾向を把握する、AI分析（12種類）で客観的な視点を得る。日記が自己分析ツールになります。',
  ...
  const data = {
    ...
    mainEntity: [
      ...
      },
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: '年代別要約、頻出感情タグ抽出、文章トーン分析、転機検出、反復テーマ抽出、内省質問の生成、季節別感情マップ、成長分析、包括レポート、標高ナラティブ、強みの宣言、反事実的因果分析の12種類の分析に対応しています。ユーザー自身のOpenAI APIキーを使用するため、データがサービス提供者に渡ることはありません。',
        },
      },
      {
        '@type': 'Question',
