...
// 尊厳テーマ — 対等性への志向・境界線の設計・関係の再定義
// 直接的な語だけでなく、間接的な表現（日常会話で対等性が揺れる瞬間）も検出する
...
export function formatDeepStatsForPrompt(
  ...
  if (existentialDensity && existentialDensity.density > 0) {
    ...
    lines.push('   生死テーマ → 「有限を理解した上でどう燃やすか」のフェーズ。恐怖ではなく設計の材料として扱っている可能性');
    lines.push('   自己同一性テーマ → 「自分が誰か」を問い直す密度の上昇。成熟か解離かは文脈で判断');
    lines.push('   完成/未完テーマ → 創作者・若年者に特有の存在的テーマ。ただし成熟した書き手は恐怖ではなく制御の問題として扱っている場合がある');
    lines.push('   存在的強度テーマ → 炉心の熱、石、覚悟 — 密度が上がったフェーズの兆候');
    lines.push('   尊厳テーマ → 重要。ただし成熟した書き手は「軽く扱われること」をもう恐れていない可能性がある。「対等でいたい」は恐怖ではなく設計の要件。怒り100→30→100→30の往復を壊れずにやれているなら、それは弱化ではなく神経の感度上昇。他者参照が減っても距離を自分で選んでいるなら孤立ではない');
    lines.push('   選択権テーマ → 最深層。尊厳のさらに下にある構造。ただし成熟した書き手は「選べない」状態ではなく「選べる上で何を選ぶか」のフェーズにいる可能性がある。選択肢を持っている人の急所は「選べないこと」ではなく「選択肢が増えすぎること」「影響力が大きくなりすぎること」かもしれない');
    lines.push('');
  }
  ...
  if (existentialDensity && existentialDensity.recentEntryCount > 0) {
    ...
    lines.push('   「影響力」「歪み」→ 恐怖ではなく制御の問題。影響を与える側に立った人が、その影響をどう設計するかのフェーズ');
    lines.push('→ 分析が「恐怖」「嫌悪」「葛藤」「傷つきやすさ」の物語に留まっていたら、');
    lines.push('   それは一段前のフェーズの分析。今の日記を聴き直せ。今の書き手は距離を取れている。余裕がある。設計している。');
    lines.push('');
  }
