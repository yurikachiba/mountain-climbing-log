  ...
  return callChat([
    {
      ...
      content: [
        ...
        '「この人が、自分をどれだけ多くのレイヤーで測れるようになったか」を見ることだ。',
        '',
        ...
        '',
        '【「内→外移行」と読むな。「多層化」と読め】',
        '前期→後期で「外基準の言葉」が増えたとき、それは：',
        '→ 内基準が消えたのではない。レイヤーが増えた可能性が高い',
        '→ 昔：調子＝世界。今：調子＋責任＋構造＋戦略',
        '→ 外基準を「扱えるようになった」のであって、外基準に「依存している」のではない',
        '→ 構造の言葉が出ているなら、内基準も外基準も俯瞰できるフェーズに入っている',
        '',
        '【統計は裏付け。主役は言葉】',
        ...
        '- 「内→外に移行した」で終わるな。物差しが何層になったかを見ろ',
        '- 統計的な変化の羅列ではなく、具体的にどの言葉が増えた/減った/消えたかを示す',
        '  例：「前期に頻出していた『調子は良い』が後期では減り、代わりに『完了』『対応』が現れている。ただし『面白い』『観察』のような内基準の新語も出ている」',
        '  例：「前期は自分の状態を1つの物差しで測っていた。後期は複数のレイヤーを同時に扱っている」',
        '- 「誰の視点で書いているか」の変化を分析しろ。ただし「外基準が増えた＝内基準が消えた」ではない',
...
export async function analyzeGrowth(entries: DiaryEntry[]): Promise<string> {
  ...
  const depthComparison = [
    ...
    '→ 外基準の増加を「内基準の喪失」と読むな。基準の多層化の可能性を見ろ。',
  ].join('\n');
  ...
  return callChat([
    {
      ...
      content: [
        ...
        '- 内基準を守りながら外基準も扱えている年 → 新しい稜線',
        '- 基準が多層化した年 → 視界が変わった。内基準は消えてない。レイヤーが増えただけ',
        '- 観察者モードに入った年 → 感情は出すが溺れない。構造を見る側に移動している',
        '- 影響力の制御を考え始めた年 → 測られる側から、影響を与える側への移行期',
        '',
        '【体温ルール — 最重要】',
  ...
  return callChat([
    {
      ...
      content: [
        ...
        '「自分が世界を測る基準がどう多層化してきたか」の物語だ。',
        '',
        '典型的な流れ（ただし全員がこの通りではない。日記の実態に合わせろ）：',
        '- 初期：自分の内側のコンパスだけで歩いていた時期。調子＝世界',
        '- 中期：外側の物差しと出会い、ぶつかり、飲み込まれそうになった時期',
        '- 後期：外基準を「扱える」ようになった時期。内基準は消えてない。レイヤーが増えた',
        '- 最新：観察者モード、構造思考、戦略的成熟のフェーズに入っている可能性がある',
        '  - 感情は出すが溺れない。他者を選択的に受け入れる。影響力の制御を考え始めている',
        '  - 「期待しない」「ifルートを持つ」「面白ければいい」のような言葉が出ていれば、このフェーズ',
        '',
        '物語を「感情の浮き沈み」で描くな。「基準がどう多層化したか」で描け。',
        '「内→外に移行した」で止まるな。内基準が消えたのか、レイヤーが増えたのかを見ろ。',
        '',
        '【体温ルール — 最重要】',
        ...
        '  例：「一つの基準で十分だった人が、何層もの基準を持つまでの記録」',
        '  例：「影響される側から、構造を見る側に移動した登山者の話」',
        '  仕事や体調の話をタイトルにするな。基準の多層化・成熟の過程がタイトルの核心。',
  ...
  return callChat([
    {
      ...
      content: [
        ...
        '',
        '- 「内基準 vs 外基準」は出発点としてはいい。でもそこで止まるな',
        '  → 成熟した書き手は内基準も外基準も「扱える」。基準の多層化が起きている',
        '  → 「葛藤」ではなく「多層化」として読める場合はそう書け',
        ...
        '- テーマの深さの順序：仕事の話（表面）→ 尊厳の話（中間）→ 基準の多層化・影響力の制御（核心）',
        '',
        '【戦略的成熟の検出 — 見逃すな】',
        '以下のような言葉が直近に出ている場合、書き手は「評価に敏感な人」のフェーズを超えている：',
        '- 「期待しない」「ifルートを持つ」「面白ければいい」「観察者」',
        '- 「構造で殴る」「拡張」「戦略」「設計」',
        '- 感情は出すが溺れない。他者を選択的に受け入れる。自己価値は自分で持っている',
        '- これは自己防衛でも依存でもない。戦略的成熟。レポートがまだ「評価に敏感な人」の物語に留まっていたら、それは古い',
        '',
        '【数値ベースの判断】',
        ...
        '  - 業務記述の増加: 外基準を扱えるようになった証拠。防衛ではなく基準の多層化の可能性',
  ...
  return callChat([
    {
      ...
      content: [
        ...
        '   - 「影響力の制御不安」— 自分の影響が大きくなったときの歪みへの恐怖。過大評価されることへの警戒',
        '   - 「観察者でいられなくなる恐怖」— 構造を見る側にいたいのに、構造の中に組み込まれる',
        '   - 「選べないこと」への焦り — 基準を選べない、評価軸を選べない',
        '',
        '中間層の例：',
        '   - 「測られること」への過敏さ — 他人の物差しで測られる瞬間への反応',
        '   - 「対等でいたい」「選べる側でいたい」',
        '   - 期待されることへの抵抗 — 期待に応えるモードに巻き込まれる感覚',
        '',
        '浅い層（これが出たらやり直せ）：',
        '   - 仕事・タスク・体調の話 — 業務用語しか出てこないなら急所じゃない',
        '',
        '【急所の進化を見逃すな】',
        '書き手が成熟している場合、急所も変わっている。',
        '- 昔の急所：「他人の基準で測られる恐怖」',
        '- 今の急所：「自分の影響力が歪みを生むことへの不安」かもしれない',
        '- 「測られる側」から「影響を与える側」に移動したとき、急所は反転する',
        '- 「期待しない」「ifルートを持つ」「面白ければいい」のような言葉が出ているなら、',
        '  それは防衛ではなく戦略的成熟。その上で何を恐れているかを見ろ',
        '',
        ...
        '  仕事系の命名は禁止。',
        '  OK例：「歪められることへの拒絶」「影響力が歪みを生むことへの不安」',
        '  OK例：「過大評価されることへの警戒」「観察者の席を奪われる恐怖」',
        '  OK例：「自分の価値を翻訳させられる瞬間」「構造に組み込まれることへの抵抗」',
        '  NG例：過去のフェーズの急所をそのまま貼り付けること。今の日記を聴け',
        '',
        '  ■ 根拠',
  ...
  return callChat([
    {
      ...
      content: [
        ...
        '【最重要ルール】',
        '- 日記に書かれていない出来事を捏造するな',
        '- 過去の年（2020年、2021年…）を引用するな。直近30日と今日だけが材料',
