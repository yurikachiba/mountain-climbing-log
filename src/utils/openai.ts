  ...
  return callChat([
    {
      ...
      content: [
        ...
        '',
        '【選択権レイヤーの検出 — 尊厳のさらに下】',
        '尊厳の下に「選択権」がある。これが最も深い層：',
        '',
        '  表面: 未完（タスクが終わらない）',
        '  中間: 尊厳（未完だと測られる恐怖）',
        '  核心: 選択権（基準も評価軸もルールも選べない）',
        '',
        '以下のパターンが見えたら、急所は「選択権」にある：',
        '- 「普通」という基準に怒る → 基準を選べない',
        '- 「意味のあることを喋って」に怒る → 評価軸を選べない',
        '- 「英語デフォルト」「多数派」「構造」に怒る → ルールを選べない',
        '- お金に関する記述が「贅沢」ではなく「選択肢」として書かれている',
        '- 「離れたい」「辞めたい」「距離を取りたい」→ 縛られている感覚への反応',
        '',
        '選択権が核心のとき：',
        '→ 書き手は支配したいのではない。縛られたくない',
        '→ 怒りは「不当な扱い」だけでなく「そこから離れられない」構造への怒り',
        '→ 急所の命名を「尊厳」でも「未完」でもなく「選べない」「縛られる」「自由」系にしろ',
        '→ 処方箋は「怒りを手放せ」ではなく「選択肢を増やす方法は戦う以外にもある」の方向',
        '→ 例（尊厳的）：「高度を他人に測られるのを拒む体質」',
        '→ 例（選択権的）：「ルートを決められることが最も息が詰まる」「自分の足で選んだ道しか歩けない体質」',
        '',
        '【関係性レイヤーの検出】',
        ...
        '  例（選択権的）：「ルートを決められると息が詰まる体質」「自分の足で選んだ道しか歩けない」「他人の地図で登らされる嫌悪」',
        '',
        '  ■ 根拠',
