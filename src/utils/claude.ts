  ...
  return callChat([
    {
      ...
      content: [
        ...
        'Step 2: 各候補を「出来事」「構造変化」「気づき」「到達」に分類しろ',
        ...
        'Step 3: 到達があれば最優先。構造変化・気づきがあればそれも候補。出来事は全部却下',
        ...
        '',
        '  ■ 候補',
        '  日記から3〜5個の候補を挙げ、各候補を「出来事」「構造変化」「気づき」「到達」に一言で分類。',
        '  到達・構造変化・気づきがあるのに出来事を急所に選んだら、その出力は全部やり直し。',
        '  さらに、候補の中で「今日初めて書かれたこと」「今まで書いてこなかったこと」があるなら、それに★をつけろ。',
        '  ★がついた候補は最優先。新規性が高い候補を上位にしろ。',
        '  「到達」がある場合、それが最優先。',
        '',
        '  ■ 急所：[一言で命名。メタファーなし、ストレートに]',
        '  ■ 候補 で「到達」「構造変化」「気づき」に分類したものから選べ。「出来事」に分類したものは選ぶな。',
...
// 文体気象図 — 感情状態ごとの文体パターン（語彙・比喩・文長）を複数エントリから横断的に抽出する
export async function analyzeStyleWeather(entries: DiaryEntry[]): Promise<string> {
  if (entries.length === 0) return '';

  const sorted = [...entries].filter(e => e.date).sort((a, b) =>
    (a.date ?? '').localeCompare(b.date ?? '')
  );
  if (sorted.length < 5) return '文体気象図には最低5件の日記が必要です。';

  // 全期間から均等にサンプリング（最大60件）
  const sampled = sampleUniform(sorted, 60);
  const entriesText = sampled.map(e => `[${e.date}] ${e.content.slice(0, 400)}`).join('\n---\n');

  return callChat([
    {
      role: 'system',
      content: [
        'あなたは日記の文体を読む人。内容ではなく、書き方を読む。',
        '',
        '【この分析の目的】',
        '他の分析は「何を書いたか」を読む。この分析は「どう書いたか」を読む。',
        '感情の状態が変わると、文体が変わる。それを見つけ出す。',
        '',
        '具体的に観察するもの：',
        '- 文の長さ（短く切れる日 vs 長く繋がる日）',
        '- 比喩の種類（自然・身体・空間・機械・食べ物…）',
        '- 語彙の温度（硬い言葉が増える日、柔らかい言葉が増える日）',
        '- 句読点の打ち方、体言止めの頻度、疑問文の出現',
        '- 一人称の使い方の変化',
        '- 書き出しのパターン（いきなり本題 vs 天気や状況から）',
        '',
        '【出力形式】マークダウン記法（#, ##, ###, ** 等）は使うな。■ を見出しとして使え。',
        '',
        '【最重要ルール】',
        '- 内容の要約をするな。文体の観察だけをしろ',
        '- 「成長した」「乗り越えた」は書くな',
        '- 日記に書かれていない出来事を捏造するな',
        '- 日記の言葉を「」で引用して根拠を示せ',
        '- 登山メタファーは使うな',
        '- パターンが見つからない場合は正直に「見つからない」と書け',
        '',
        '【主語を間違えるな】',
        '- 書き手が他者に言った言葉の内容は、書き手自身の内面ではない',
        '- 他者の行動を書き手の行動として読むな',
        ...
        '',
        '■ 荒天の文体',
        '感情が荒れている日・不安が強い日・怒りの日の文体パターン。',
        '文の長さ、語彙、比喩、句読点の打ち方に何が起きるか。',
        '該当する日記から具体的に「」で引用して示せ。',
        '',
        '■ 凪の文体',
        '穏やかな日・満足している日・楽しい日の文体パターン。',
        '荒天の日と何が違うか。',
        '',
        '■ 霧の文体',
        '考え込んでいる日・迷っている日・判断がつかない日の文体パターン。',
        '荒天とも凪とも違う、第三の文体があるか。',
        '',
        '■ この人の文体の指紋',
        '感情状態に関わらず一貫している文体の特徴。',
        '「この人の書き方だ」と分かる癖・習慣。',
        '',
        '- 各セクション3〜5文。長い段落を書くな',
        '- 全体で800〜1500字',
    ...
    },
    {
      role: 'user',
      content: [
        '以下の日記群から、感情状態ごとの文体パターンを読み取ってください。',
        '内容ではなく「書き方」に注目してください。',
        '',
        `【日記（${sampled.length}件）】`,
        entriesText,
      ].join('\n\n'),
...
// 反復の地図 — 繰り返し現れるテーマ・言い回し・状況を複数エントリから横断的にマッピングする
export async function analyzeRepetitionMap(entries: DiaryEntry[]): Promise<string> {
  if (entries.length === 0) return '';

  const sorted = [...entries].filter(e => e.date).sort((a, b) =>
    (a.date ?? '').localeCompare(b.date ?? '')
  );
  if (sorted.length < 10) return '反復の地図には最低10件の日記が必要です。';

  // 全期間から均等にサンプリング（最大80件、広く拾う）
  const sampled = sampleUniform(sorted, 80);
  const entriesText = sampled.map(e => `[${e.date}] ${e.content.slice(0, 300)}`).join('\n---\n');

  return callChat([
    {
      role: 'system',
      content: [
        'あなたは日記の反復を見つける人。一度しか出ないものではなく、何度も帰ってくるものを見つける。',
        '',
        '【この分析の目的】',
        '日記を長く書いていると、同じテーマ・同じ言い回し・同じ状況が繰り返し現れる。',
        '本人は気づいていないかもしれない。',
        'この分析は、その「繰り返し」を地図にする。',
        '',
        '反復とは：',
        '- 同じ話題が時間をおいて何度も出てくる',
        '- 同じ比喩・言い回しが別の文脈で使われている',
        '- 同じ人物との同じパターンの関係性が繰り返されている',
        '- 同じ種類の状況に同じ反応をしている',
        '- 同じ問いが形を変えて何度も出てくる',
        '',
        '【出力形式】マークダウン記法（#, ##, ###, ** 等）は使うな。■ を見出しとして使え。',
        '',
        '【最重要ルール】',
        '- 反復が2回以上確認できるものだけを報告しろ。1回きりのものは反復ではない',
        '- 日記から「」で引用して、反復の証拠を示せ（異なる日付から）',
        '- 反復を良い・悪いと評価するな。繰り返しの事実だけを描け',
        '- 日記に書かれていない出来事を捏造するな',
        '- 登山メタファーは使うな',
        '',
        '【主語を間違えるな】',
        ...
        '【禁止フレーズ】',
        '「成長の証」「未来への一歩」「素晴らしい」「立派」「頑張った」「乗り越えた」',
        '「パターンを手放す」「気づきを得る」— 処方箋は出すな',
        '',
        '以下の形式で出力する：',
        '',
        '■ 帰ってくる問い',
        '時間をおいて繰り返し現れる問い・テーマ・悩み。',
        '異なる日付の日記から「」で引用して、同じ問いが帰ってきていることを示せ。',
        '問いの形が変わっていても、核が同じなら同じ反復として扱え。',
        '',
        '■ 繰り返す場面',
        '似た状況・似た人間関係のパターンが繰り返されている場面。',
        '「また同じことが起きている」ではなく、具体的にどの日記とどの日記が似ているかを示せ。',
        '',
        '■ 定型句',
        '書き手がよく使う言い回し・比喩・口癖。',
        '同じ表現が複数の日記に出てきていることを「」で引用して示せ。',
        '些細なものでもいい。本人の「手癖」が分かるもの。',
        '',
        '■ 変奏',
        '同じテーマが時間とともに形を変えている場合。',
        '最初はAだったものが、後にBになっている。反復しつつ変化しているもの。',
        'なければ省略可。',
        '',
        '- 各セクション3〜5文',
        '- 全体で800〜1500字',
        '- 見つからないセクションは省略可。無理に埋めるな',
      ].join('\n'),
    },
    {
      role: 'user',
      content: [
        '以下の日記群から、繰り返し現れるテーマ・言い回し・状況を見つけてください。',
        '一度きりのものではなく、何度も帰ってくるものだけを報告してください。',
        '',
        `【日記（${sampled.length}件、${sorted[0]?.date ?? ''}〜${sorted[sorted.length - 1]?.date ?? ''}）】`,
        entriesText,
      ].join('\n\n'),
    },
  ], 2500);
...
// 季節の自分 — 季節・時期ごとの自分の傾向を複数エントリから横断的に抽出する
export async function analyzeSeasonalSelf(entries: DiaryEntry[]): Promise<string> {
  if (entries.length === 0) return '';

  const sorted = [...entries].filter(e => e.date).sort((a, b) =>
    (a.date ?? '').localeCompare(b.date ?? '')
  );
  if (sorted.length < 20) return '季節の自分には最低20件の日記が必要です。';

  // 季節ごとにグループ化してサンプリング
  const bySeason: Record<string, DiaryEntry[]> = { spring: [], summer: [], autumn: [], winter: [] };
  for (const e of sorted) {
    const month = parseInt(e.date!.substring(5, 7));
    if (month >= 3 && month <= 5) bySeason.spring.push(e);
    else if (month >= 6 && month <= 8) bySeason.summer.push(e);
    else if (month >= 9 && month <= 11) bySeason.autumn.push(e);
    else bySeason.winter.push(e);
  }

  const seasonLabels: Record<string, string> = { spring: '春（3-5月）', summer: '夏（6-8月）', autumn: '秋（9-11月）', winter: '冬（12-2月）' };
  const seasonTexts = Object.entries(bySeason)
    .filter(([, es]) => es.length > 0)
    .map(([season, es]) => {
      const sampled = sampleSliceFromArray(es, 15);
      const texts = sampled.map(e => `[${e.date}] ${e.content.slice(0, 300)}`).join('\n---\n');
      return `【${seasonLabels[season]}の日記（${es.length}件中${sampled.length}件を抽出）】\n${texts}`;
    })
    .join('\n\n');
  ...
  return callChat([
    {
      role: 'system',
      content: [
        'あなたは日記から季節ごとの自分の傾向を読む人。',
        '',
        '【この分析の目的】',
        '人には季節ごとの傾向がある。冬に沈みやすい人、夏に活動的になる人、秋に内省的になる人。',
        'でもそれは本人には見えにくい。',
        'この分析は、日記を季節で切り分けて、「春の自分」「夏の自分」「秋の自分」「冬の自分」を描く。',
        '',
        '観察するもの：',
        '- 話題の傾向（何について書いているか）',
        '- 感情の温度（文体が変わるか、語彙が変わるか）',
        '- 活動量（外に出ている、人と会っている、引きこもっている）',
        '- 身体への言及（体調、睡眠、食欲）',
        '- 関係性の温度（人との距離感が変わるか）',
        '',
        '【出力形式】マークダウン記法（#, ##, ###, ** 等）は使うな。■ を見出しとして使え。',
        '',
        '【最重要ルール】',
        '- 季節の傾向は、複数年にわたって確認できるものだけを報告しろ',
        '- ある年だけの特殊事情を季節の傾向として扱うな',
        '- 日記から「」で引用して根拠を示せ',
        '- 「成長した」「乗り越えた」は書くな',
        '- 日記に書かれていない出来事を捏造するな',
        '- 登山メタファーは使うな',
        '- データが少ない季節は正直に「データが少なく傾向が読めない」と書け',
        '',
        '【主語を間違えるな】',
        '- 書き手が他者に言った言葉の内容は、書き手自身の内面ではない',
        '- 他者の行動を書き手の行動として読むな',
        ...
        '「成長の証」「未来への一歩」「素晴らしい」「立派」「頑張った」「乗り越えた」',
        '',
        '以下の形式で出力する（日記がある季節のみ）：',
        '',
        '■ 春の自分',
        '春（3-5月）の日記に見える傾向。話題、温度、活動量、身体、関係性。',
        '日記の言葉を「」で引用して示せ。',
        '',
        '■ 夏の自分',
        '夏（6-8月）の日記に見える傾向。',
    ...
    },
    {
      role: 'user',
      content: [
        '以下の日記を季節ごとに読み、それぞれの季節での書き手の傾向を描いてください。',
        '',
        seasonTexts,
      ].join('\n\n'),
    },
